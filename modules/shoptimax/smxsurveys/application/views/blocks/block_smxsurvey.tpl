...
[{block name="smxsurveyblock"}]
[{if $survey && !$surveyblockrendered && $oView->getClassName() ne "content" && $oView->getClassName() ne "tpl"}]
...
<div class="surveybox">
    <h3>[{$survey->smxsurveys__title->value}]</h3>
    [{if $alreadyvoted}]
        ...
        [{else}]
                    ...
                    <td valign="top">
                        ...
                        [{foreach from=$question->getPossibleAnswers() item=answer name="allanswers" }]
                            ...
                            [{if $answer->smxsurveys_answers2questions__is_freetext->value == "1"}]
                                <br />
                                <input type="text" value="" maxlength="255" size="20" name="smxanswersfreetext[[{$answer->smxsurveys_answers2questions__oxid->value}]]" />
                            [{/if}]
                            <br />
                        [{/foreach}]
                    </td>
            ...
            <tr>
                <td><br />
                    ...
                    <br />
                    <input type="text" name="useremail" size="20" maxlength="255" value="[{$oxcmp_user->oxuser__oxusername->value}]" />
                </td>
            </tr>
            [{/if}]
            <tr>
                <td><br />
                    ...
                    [{if $survey->smxsurveys__showresult->value == "1" }]
                        <input type="button" value="[{ oxmultilang ident="SMXSURVEYS_SHOWRESULT" }]" onclick="location.href='[{$oViewConf->getSelfLink()}]showresults=1&snid=[{$snid}]&cl=[{$shop->cl}][{if $product}]&anid=[{ $product->oxarticles__oxnid->value }][{/if}]&cnid=[{$shop->cnid}]';" />
                    [{/if}]
                </td>
            </tr>
            </table>
...
[{assign var="surveyblockrendered" value="true"}]
[{/block}]
